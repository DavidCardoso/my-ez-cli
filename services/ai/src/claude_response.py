...
def parse_claude_response(raw_json: str, logger: logging.Logger) -> tuple[str, str, int, int]:
    """
    Parse Claude Code JSON response into (session_id, result, input_tokens, output_tokens).
    ...
    Returns:
        Tuple of (session_id, result text, input_tokens, output_tokens)
    ...
    logger.debug(
        "Parsed Claude response: session_id=%s, result_len=%d, tokens=%d/%d",
        session_id,
        len(result),
        input_tokens,
        output_tokens,
    )
    return session_id, result, input_tokens, output_tokens
...
def write_ai_analysis(
    ...
    Args:
        ...
        logger: Logger instance
        input_tokens: Number of input tokens used in the Claude request
        output_tokens: Number of output tokens generated by Claude
